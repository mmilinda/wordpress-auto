 ...
 * @since  1.0.0
 */
class Themehunk_Library_Options_Import {
		...
		if ( isset( $cats ) ) {
			...
			foreach ( $cats as $key => $cat ) {
				...
				if ( ! empty( $cat['slug'] ) && ! empty( $cat['thumbnail_src'] ) ) {
					...
					$image = (object) Themehunk_Library_Helper::_sideload_image( $cat['thumbnail_src'] );
	...
	private function insert_logo( $image_url = '' ) {
		...
		$data = (object) Themehunk_Library_Helper::_sideload_image( $image_url );
